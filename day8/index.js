...
const getLayers = (image, width, height) => {
  const layers = [];
  while (image.length > 0) {
    layers.push(image.splice(0, width * height));
  }
  return layers;
};
...
const spaceImageEncoder = (image, width, height) => {
  ...
  const lowestZeros = counts.sort((a, b) => a[0] - b[0])[0];
  return lowestZeros[1] * lowestZeros[2];
};

//console.log(spaceImageEncoder(inputs, 25, 6));
/*
--- Part Two ---
