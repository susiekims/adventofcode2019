...
const amplifierFeedbackLoop = (program, phases) => {
  let maxSignal = 0;
  ...
  const runPhase = (phase, input = 0) => {
    const ampA = Intcode(programA, [phase[0], input]);
    const ampB = Intcode(programB, [phase[1], ampA]);
    const ampC = Intcode(programC, [phase[2], ampB]);
    const ampD = Intcode(programD, [phase[3], ampC]);
    const ampE = Intcode(programE, [phase[4], ampD]);
    runPhase(phase, ampE);
  };

  for (let i = 0; i < phases.length; i++) {
    console.log(i);
    const output = runPhase(phases[i], 0);
    if (output > maxSignal) {
      maxSignal = output;
    }
  }
