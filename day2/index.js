...
const runProgram = input => {
  for (let i = 0; i < input.length; i += 4) {
    ...
    const val1 = input[input[i + 1]];
    const val2 = input[input[i + 2]];
    const position = input[i + 3];

    if (opcode === 99) return input;

    if (opcode === 1) {
      input[position] = val1 + val2;
    }
